...
def draw_parallel_lines(n, point, length, window):
    ...
    if n>0:
        for k in range(n):
            yoffset=30*k+point.y
            line=rg.Line(rg.Point(point.x,yoffset),rg.Point(point.x+length,yoffset))
...
def draw_lines(n, point, window):
    ...
    if n>2:
        for k in range(n):
            yoffset=100-(200/(n-1))*k
            line=rg.Line(point,rg.Point(point.x+100,point.y+yoffset))
            line.attach_to(window)
        window.render()
        window.close_on_mouse_click()
    # -------------------------------------------------------------------------
